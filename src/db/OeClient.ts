...
class OEClient {
  ...
  public init(): Promise<any> {
    return this.runProc().then((resolve) => {
      ...
      this.client.connect(this.port, this.host, () => {
        console.log(
          "V1: TCP connection established with the server at " +
            this.port.toString() +
            "."
        );
      });
      ...
      this.client.on("data", (chunk) => {
        console.log(`V1: Data received from the server`);
      ...
      this.client.on("end", () => {
        console.log("V1: Requested an end to the TCP connection");
      });
  ...
  private runProc(): Promise<any> {
    return new Promise((resolve) => {
      ...
      this.proc.on("exit", (code, signal) => {
        console.log(
          "V1: child process exited with " + `code ${code} and signal ${signal}`
        );
      });

      this.proc.on("error", (error) =>
        console.log("V1: child process error: \n", error)
      );

      this.proc.stdout.on("data", (data) => {
        console.log(`V1: child stdout:\n${data}`);
  ...
  public sendCommand(cmd: string): Promise<string> {
    ...
    try {
      return new Promise((resolve) => {
        console.log("V1: Sending request to server");
        this.client.write(`${cmd}\n`);
        this.dataFinish = resolve;
      });
    } finally {
      console.log("V1: Request sent");
    }
  }
